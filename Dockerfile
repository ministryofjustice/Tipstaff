...
ENTRYPOINT ["powershell", "-Command", "$DB_NAME = [Environment]::GetEnvironmentVariable('DB_NAME', 'Process'); $RDS_HOSTNAME = [Environment]::GetEnvironmentVariable('RDS_HOSTNAME', 'Process'); $RDS_PASSWORD = [Environment]::GetEnvironmentVariable('RDS_PASSWORD', 'Process'); $RDS_PORT = [Environment]::GetEnvironmentVariable('RDS_PORT', 'Process'); $RDS_USERNAME = [Environment]::GetEnvironmentVariable('RDS_USERNAME', 'Process'); $ida:ClientId = [Environment]::GetEnvironmentVariable('ida:ClientId', 'Process'); [Environment]::SetEnvironmentVariable('DB_NAME', $DB_NAME, 'Machine'); [Environment]::SetEnvironmentVariable('RDS_HOSTNAME', $RDS_HOSTNAME, 'Machine'); [Environment]::SetEnvironmentVariable('RDS_PASSWORD', $RDS_PASSWORD, 'Machine'); [Environment]::SetEnvironmentVariable('RDS_PORT', $RDS_PORT, 'Machine'); [Environment]::SetEnvironmentVariable('RDS_USERNAME', $RDS_USERNAME, 'Machine'); [Environment]::SetEnvironmentVariable('ida:ClientId', $ida:ClientId, 'Machine'); Start-Service W3SVC; Invoke-WebRequest http://localhost -UseBasicParsing; iisreset /restart; while ($true) { Start-Sleep -Seconds 3600 }"]
