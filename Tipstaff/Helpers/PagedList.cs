...
namespace Tipstaff
{
    public class xPagedList<T> : IEnumerable<T>
    {
        private IEnumerable<T> list;
        public xPagedList(IList<T> items, int pageIndex, int pageSize)
        {
            PageSize = pageSize;
        ...
        public xPagedList(IEnumerable<T> items, int pageIndex, int pageSize, int totalItemCount)
        {
            list = items;
